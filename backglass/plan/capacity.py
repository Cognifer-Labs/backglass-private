...
#: `NOT EXISTS` rather than a status column, because `source_item` is immutable and the
#: row stays true forever: the class really did meet on Wednesdays until the 10th. What
#: changed is that Calendar.app no longer has it, which migration 0030 records beside the
#: item. Without this join the planner keeps subtracting dropped classes — five of them,
#: for ten days, which is what sent the owner looking.
#:
#: The retraction join is deliberately outside the indexed predicate. It filters a handful
#: of rows the range read already found, so it costs a lookup per calendar event rather
#: than a second scan, and the `datetime(si.occurred_at)` bounds still bind the expression
#: index — which the plan test asserts, because that is not obvious from reading it.
CALENDAR_DAY_SQL = (
