...
def promote_orphans(
    conn: sqlite3.Connection, settings: Settings, *, dry_run: bool = False
) -> tuple[int, list[str]]:
    """Assignments the extractor read and said nothing about.

    Owner, 2026-08-27: "other assignments havent been schedul[ed]". Measured on the live
    ledger the same afternoon: `assignment` held 205 rows, 183 carried a commitment, and
    **22 did not** — Week 3, 4, 11, 12 and 14 of HON 171, both Participation Grades, six
    PSY 101 practice quizzes, seven LearningCurves, two CIS 236 practice sets, a CHM 113
    VR pod session. Every one of them was triaged `keep` and run through
    `extract-commitments@10`. The model read them and emitted nothing.

    The flaw is not that the model was wrong on twenty-two rows; it is that it was asked
    at all. **A Canvas assignment is a commitment by construction.** The feed states the
    deliverable, the course and the deadline, `coursework` has already read the effort off
    the description, and there is no judgement left to make. Routing that through a
    language model buys nothing and costs the one failure mode neither the ledger nor the
    owner can see: rule 2 sends a low-confidence extraction to the review queue, and this
    said *nothing* — no row, no question, no count. Twenty-two pieces of graded work were
    simply not there.

    So the dated assignments are promoted here, deterministically, and confidence is 1.0
    because nothing was inferred.

    Undated rows are left alone. Canvas carries course pages and ungraded material in the
    same feed, and an obligation with no date is one the planner cannot place and the
    owner cannot miss — inventing four hundred of them would bury the board rather than
    fill it.
    """
    from backglass.ledger import Ledger

    rows = conn.execute(
        "SELECT a.id, a.title, a.course, a.due_at, a.effort_minutes, a.source_item_id "
        "FROM assignment a "
        "WHERE a.user_id = ? AND a.due_at IS NOT NULL AND a.due_at != '' "
        "  AND a.source_item_id IS NOT NULL "
        "  AND NOT EXISTS (SELECT 1 FROM commitment c "
        "                  WHERE c.source_item_id = a.source_item_id) "
    ...
    open_now = conn.execute(
        "SELECT what, due_at FROM commitment WHERE user_id = ? AND status = 'open'",
        (USER_ID,),
    ).fetchall()
    ...
    notes: list[str] = []
    for row in rows:
        due = str(row["due_at"])[:10]
        if _already_on_the_board(str(row["title"]), due, open_now):
            # The same quiz can reach the ledger twice — once from the Canvas feed and
            # once from the mail notification about it — and those are different
            # `source_item`s, so the join above cannot see it. Skipped rather than
            # merged: `duplicates` is the surface built to ask that question, and a bulk
            # insert that guesses wrong puts the owner's board out by twenty-two rows.
            notes.append(f"skipped (already on the board): {row['title'][:60]}")
            continue
        promoted += 1
        notes.append(
            f"{due}  {row['title'][:56]}"
            + (f"  ({row['effort_minutes']}m)" if row["effort_minutes"] else "")
        )
        if dry_run:
            continue
        ledger.insert_commitment(
            ...
            what=str(row["title"]),
            due_at=due,
            estimated_minutes=int(row["effort_minutes"]) if row["effort_minutes"] else None,
            # `analyzed` is the ladder rung `apply_estimates` may overwrite and the flag
            # `planner.Candidate` reads as "this is homework, and it splits across
            # sittings". Both are true of anything that arrived this way.
            estimate_source="analyzed" if row["effort_minutes"] else None,
            confidence=1.0,
            source_item_id=int(row["source_item_id"]),
            # Rule 1. The evidence is the feed's own words for it — course and title —
            # which is exactly what the owner would check the claim against.
            evidence=f"{row['course']} — {row['title']}".strip(" —"),
            evidence_kind="original",
        )
    return promoted, notes


def _already_on_the_board(title: str, due: str, open_now: Sequence[Any]) -> bool:
    """Is this work already an open commitment under another source?

    Identity is the due date **and** the words, and the date is the half that matters.
    Titles alone cannot do it: "Chapter 4 Practice Quiz" and "Chapter 14 Practice Quiz"
    share five of six words and are different assignments five weeks apart, and on the
    live ledger a title-only check called eight of the twenty-two orphans duplicates and
    every one of those was wrong. With the date in, the same run found zero.
    """
    if not due:
        return False
    wanted = _words(title)
    if not wanted:
        return False
    for row in open_now:
        if str(row["due_at"] or "")[:10] != due:
            continue
        other = _words(str(row["what"]))
        if other and len(wanted & other) / len(wanted | other) >= 0.5:
            return True
    return False
