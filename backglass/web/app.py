...
def serve(
    ...
    # `_warm` before the loop, not after: both reach `dedup.suspects`, whose wording
    # scores memoise across calls, and the `duplicates` pass pays ~3.7 s for them on a
    # cold cache. Warming first means the loop's thread finds them already there instead
    # of the two racing to compute the same scores.
    _warm(resolved)
