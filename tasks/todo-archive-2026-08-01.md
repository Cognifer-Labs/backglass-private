...
# Backlog — next todos (2026-08-01)

Collected from every "recorded, not done" thread above. Nothing here is new scope;
each line already has a paper trail. Ordered by leverage.

## Hygiene / correctness debt

- [x] **iMessage WAL open-mode fix** — done 2026-08-01: `mode=ro` +
      `busy_timeout=2000`, docstring + comment updated, WAL-only-rows regression
      test ported from anki/avorio (proved discriminating: immutable open sees 1
      row where mode=ro sees 2). Real chat.db unprobeable from this shell (no
      Full Disk Access) — relied on Phase A verifier's "chat.db is also WAL".
- [ ] **Separate + commit the held work** — cheap-acquisition arc (7 phases),
      med-student Phase A/A2, instagram connector, format audit, design audit
      are all uncommitted and intermingled in shared files. Untangle into
      grouped commits per arc (Phase 8 S0 precedent), owner authorship, no push
      without remote check. Run `dk-commit` preflight first.
- [x] **`entity.kind` resolver-aware reclassification** — done 2026-08-01:
      resolver matches `kind IN ('person','org')` (flipped rows keep resolving,
      no fragmentation); an owner `org`/`person` tag now writes through to
      `entity.kind` in person_update/person_create (IntegrityError on a
      cross-kind name collision → 422, not 500); `org_like` reads persisted kind
      first, then tags, then heuristic. Side effect (desirable, by construction):
      kind='org' rows drop out of going-cold nudges (`people_cold.sql` filters
      person) — a portal never "goes cold". 3 new tests; 630 green.

## Product path (fulfils the ledger purpose directly)

- [ ] **Phase 13 — fact extraction at ingest** — the `fact` table's
      `source='extraction'` column is ready (Phase 12 recorded this as the
      natural next phase). Needs its own versioned prompt under
      `specs/extraction-prompts/`, fixture set per the testing rules, review
      queue for low confidence, and the same idempotency guarantee.
- [ ] **AMCAS export report page** — CLI `backglass amcas-export` proved the
      assembly (Phase A ruling deferred the page). Add a read-only /amcas page
      rendering the same evidence: per-activity hours citing checkpoint ids,
      char counts vs 700/1325, no application prose written for the owner.
- [ ] **Med PRD F3 — application cycle tracker** — schools/secondaries/
      interviews as append-only events, extraction-driven (docs/14 P1 after
      F1/F2 which are built). Needs the §7 owner rulings that remain open.
- [ ] **Med PRD F4 — `metric` target kind** — MCAT FL score trajectory; the
      totals machinery (0006) is the template: kind + value column +
      progress-on-read, no cached counters.

## Activation (owner-blocked, keep visible)

- [ ] **Runbook steps 2–10** — `docs/13` + `backglass doctor`'s remaining
      failures ARE the checklist: OAuth consent, `.env` fill, launchd load.
      Everything downstream (7-day soak, cassette recording, Phase 1/2/5 exit
      criteria) unblocks on this one owner session.
- [ ] **Record vcrpy cassettes** — first action after OAuth has run once;
      replaces the hand-written FakeGmailService fixtures with real shapes.

## Polish (bounded, deliberately deferred passes)

- [x] **Small-type ramp sweep** — done 2026-08-01: all 16 text sites at 10px
      raised to the scale's 11px label floor (line-heights 13→14, 12→13, code
      11/15); geometry-only 10px values (svg, check glyph, thin track) left.
      Verified live on demo db (DB_PATH=backglass-demo.db, real db never
      served), both themes: week grid gutter/events, goals, dashboard all
      clean; demo open-count 4→4, theme restored to light. Palette validator +
      94 web tests green.
- [ ] **Week-grid event hit targets** — 6px blocks recorded as accepted risk;
      revisit only if the owner actually uses week view for navigation.
