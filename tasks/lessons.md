...
- 2026-08-24 | A rollback of the owner's live ledger was verified by comparing table row
  ...
  Evidence from the redone check, kept because the numbers are the argument: `day_plan` byte-identical after the rewrite, 34 tables unchanged entirely, and the only pre-existing rows that moved were `credential.cursor/updated_at` and `monitored_chat.last_seen_at/messages_seen`. The control that made it evidence: `+1` to one `planned_minutes` on a copy, and the comparison reported `day_plan: rows CHANGED`.

2026-09-08 | Built `scripts/browser_task_runner.py` to start/stop `backglass dashboard` for browser-automation sessions, claimed a `try/finally` around the child process was a cleanup guarantee, and shipped `Popen(["uv","run","backglass","dashboard"])`. Testing the interrupt path (not reading the code) found four separate holes: a squatted port health-checks as "up" on a generic 200; nested `uv run` forks rather than execs, so `terminate()` only kills the wrapper and orphans the real server two levels down; a backgrounded (`cmd &`) child has SIGINT pre-ignored by POSIX, so `except KeyboardInterrupt` never fires; and a bare SIGTERM — the harness's own kill-on-promote signal — hits Python's default disposition and skips `finally` entirely, no exception raised. | A `finally` block is a guarantee against exceptions raised in Python, not against a signal killing the process from outside it — SIGINT and SIGTERM both need an explicit `signal.signal()` handler that converts them into a catchable exception. Any wrapper that starts a background process needs its interrupt/kill/port-collision paths tested, not just its happy path; each of the four bugs here only a live signal/collision test surfaced. Full detail in ~/.claude memory `browser-automation-stack-decision`.
2026-09-08 | Asked to make sure the app schedules itself regardless of whether it's opened, I read `data/sync.err` by hand and found 196 repeated `MigrationError` lines: a working-tree migration rename collision (0040 recorded as `ask_gate`, disk briefly had `calendar_writeback` at the same slot) made `migrate(conn)` throw on *every* scheduled `sync` invocation — the plan/brief catch-up net (`catchup.run`) rides inside `sync()`, so this also silently disabled it. `state`'s own "last run completed without degrading" check stayed green throughout, because it only reads the *last successful* row — a crash before `INSERT INTO run` leaves no row at all, so the check was reading a healthy run from before the outage as if it were current. My first fix attempt was a time-since-last-run staleness check in `state.py`; querying the real `run` table showed 21 of the last 137 gaps exceeded an hour (up to 75h, ordinary laptop-sleep), so a fixed threshold would have cried wolf constantly and trained the check to be ignored — caught by checking real data before shipping, not by the unit test, which only proved the happy/stale cases I'd already imagined. | Prefer an outcome-based check over a time-based one when the "normal" cadence is irregular (a laptop sleeps). The actual fix: wrap `migrate(conn)` in `sync_command` so a failure writes a degraded `run` row (`sync.record_run`, already existed for this exact purpose) before re-raising — the *existing* check then catches it on the very next cycle with zero new surface. Verified by reproducing the exact incident against a scratch copy of the real DB (corrupted `schema_version.filename` for version 40, ran `backglass sync`, confirmed the degraded row and the now-red verdict), not by reasoning about it.
2026-09-08 | Two separate automation failures in one Canvas-submission task, both from the same wrong assumption — that an automation tool's input reaches its intended target as reliably as a physical action. (1) AppleScript `System Events keystroke` sends to whichever app is OS-frontmost, not to "the tab I last clicked via mcp-safari" — focus silently drifted to the terminal/IDE process between separate Bash tool calls, so a keystroke burst typed straight into the wrong window with no error, twice, dropping a clause mid-paragraph in a Google Doc. (2) A native file-picker opened fine via a `.click()` triggered through the Safari automation API (mcp-safari), and could even be driven end-to-end via the accessibility hierarchy (type-ahead select, click Upload by UI-element reference) — the dialog closed each time with no error, but WebKit never handed the chosen file back to the page's `<input type=file>`; the upload silently stayed empty. | Two mechanical rules: (a) any script that sends OS-level keystrokes/clicks must `tell application "X" to activate` and perform the interaction in the SAME script invocation — never assume frontmost status survives to the next tool call, since the harness itself can steal focus between calls. (b) A file-picker result only reaches a web page's file input when the entire gesture chain is genuine HID input — `System Events click at {x,y}` (a real synthetic mouse event) works where accessibility-hierarchy clicks and keyboard-driven selection do not, because WebKit can tell the difference even though both open the same native dialog. When a picker "closes successfully" but the page shows no file attached, that is the tell — not a timing issue, not a wrong-button guess.

2026-09-08 | Asked for a "loop command" writing "homework walkthroughs" into "vault app," resolved both nouns wrong on the first pass and wrote two proof files to the wrong folder before catching it. "Vault app" read as `backglass vault export`'s Obsidian folder because a Typer sub-app happens to be named `vault_app` in code — coincidence, not evidence; the real referent was a second, separate macOS app (`~/Downloads/VaultApp`), named as such in the owner's own global CLAUDE.md two days earlier. Then, one level in, assumed that app's rendered folder *was* `~/Documents/Backglass Vault` because the name matched and the folder existed and was populated — it wasn't; `VaultStore.defaultVaultURL()`'s actual fallback chain (`UserDefaults` → legacy `~/Downloads/htmlvault/vault` → `~/Documents/Vault`) pointed at a third folder entirely, only found by reading the Swift source and checking the real preference plist. | A folder/name that matches what you're looking for is a hypothesis, not a confirmation — when a name is doing the identifying work ("vault", "Vault app"), grep for every project with that name (`mdfind`) and read the *consuming* code's own path-resolution logic before writing anything into it, even when the first match "looks right" and even on the second hop past the first correction. Caught here by an `advisor()` call before committing to the build (flagged the vault-identity coin-flip) and a second self-check before the second write (`defaults read` against the actual app bundle id) — cheap enough that both should be table stakes whenever a physical location is inferred from a name rather than read from the code that owns it.
