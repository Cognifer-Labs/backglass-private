...
class TestStudyTime:
    ...
    def test_it_is_never_rolled_into_tomorrow(self, conn, sett: Settings) -> None:  # type: ignore[no-untyped-def]
        ...
        # Both standing kinds, and neither rolled. Asserted as a set rather than a list
        # because the day now holds a Coding block too (P21) and their order is a
        # placement detail; what this test is about is that nothing here becomes a debt.
        assert {b["kind"] for b in _blocks(conn, MONDAY)} == {"study", "coding"}
        assert report.rolled == 0


class TestCodingTime:
    """P21 and P22, owner's ruling 2026-08-24: "more home work time and coding time"."""

    def test_every_day_gets_one(self, conn, sett: Settings) -> None:  # type: ignore[no-untyped-def]
        """Unconditional, unlike Study: the owner's own building produces no commitment
        for the board to schedule, so without a standing block it is the one lane that
        never appears on a day at all."""
        _add_commitment(conn, sett, "chem problem set", minutes=60, n=1)

        proposal = planner.propose(conn, sett, MONDAY, events=[])

        coding = [b for b in proposal.blocks if b["kind"] == "coding"]
        assert len(coding) == 1
        assert int(coding[0]["minutes"]) == sett.coding_block_minutes

    def test_switching_it_off_is_one_number(self, conn, sett: Settings) -> None:  # type: ignore[no-untyped-def]
        proposal = planner.propose(
            conn, sett.model_copy(update={"coding_block_minutes": 0}), MONDAY, events=[]
        )

        assert not [b for b in proposal.blocks if b["kind"] == "coding"]

    def test_it_shrinks_to_the_largest_hole_rather_than_vanishing(
        self, conn, sett: Settings  # type: ignore[no-untyped-def]
    ) -> None:
        """P22, and the defect that motivated it. A standing block sized against the
        capacity *left over* asks for minutes the day may hold only in pieces — and a
        block that cannot be placed says nothing at all, it simply is not there. Measured
        on the owner's real 2026-08-26: 85 minutes of capacity unspent, no gap longer
        than 60, no Coding block, no note.

        Here the fixed events leave 45-minute gaps and nothing longer, against a
        90-minute setting.
        """
        ...
        # Every gap in the 09:00-18:00 window is 45 minutes or less, and none is the 90
        ...
        proposal = planner.propose(conn, sett, MONDAY, events=events)

        coding = [b for b in proposal.blocks if b["kind"] == "coding"]
        assert len(coding) == 1, "shrunk to fit, not dropped in silence"
        assert int(coding[0]["minutes"]) <= 45
        assert int(coding[0]["minutes"]) >= sett.min_block_minutes
